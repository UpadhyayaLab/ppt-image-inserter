"""
insert_pLAT_composite_synapse_montages_slides.py

CAT (left) vs FMC (right) side-by-side deck of the
pLAT/synapse/composite/1slice/ panels (actin green + pLAT magenta,
fixed contrast) across every pLAT (date, tp) group in the compiled
manifest.

Filter: `marker == "pLAT"` rows only (3 datasets: 20250418, 20250509,
20250611). 6 groups -> 6 slides (single kind, no interleaving).
...
Usage:
    python examples_and_configs/insert_pLAT_composite_synapse_montages_slides.py
"""
...
OUTPUT_PATH = (
    ...
    "CART_pLAT_montages_20260625.pptx"
)
...
# Single kind: pLAT + actin composite at synapse (fixed contrast).
KINDS = [

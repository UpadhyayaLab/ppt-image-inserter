"""
insert_pZap70_solo_synapse_montages_slides.py

CAT (left) vs FMC (right) side-by-side deck of the
pZap70/synapse/1slice/fixed_con/ panels (pZap70 alone, fixed contrast)
across every pZap70 (date, tp) group in the compiled manifest.

Filter: `marker == "pZap70"` rows only (3 datasets: 20230921, 20231003,
20250217 D3+D5). 6 groups -> 6 slides (single kind, no interleaving).

Companion to insert_pZap70_composite_synapse_montages_slides.py — same
layout but showing pZap70 alone instead of the actin+pZap70 composite.

Usage:
    python examples_and_configs/insert_pZap70_solo_synapse_montages_slides.py
"""
...
OUTPUT_PATH = (
    ...
    "CART_pZap70_solo_montages_20260625.pptx"
)
...
# Single kind: pZap70 alone at synapse, fixed contrast.
KINDS = [

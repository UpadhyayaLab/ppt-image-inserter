"""
insert_nuc_MT_slides.py

Iterates through bTub condition subdirectories and inserts one
fixed_slice_montage_nuc_lamin_merge image per blank slide into a new presentation.
The first slide of the source PPT (title/cover) is preserved; all remaining
slides are newly added blank slides with a centered title and the montage image.

Positions are rough defaults — adjust manually in PowerPoint afterwards.

Usage:
    python examples_and_configs/insert_nuc_MT_slides.py
"""

from pathlib import Path

from pptx import Presentation
from pptx.enum.text import PP_ALIGN
from pptx.util import Inches, Pt

# ---------------------------------------------------------------------------
# Configuration — edit these paths as needed
# ---------------------------------------------------------------------------

PPT_PATH = (
    "K:/FF/PPT/PPT_autogeneration/"
    "Ctrl, DZNep, bTub, Lamin B, 20260203/"
    "Ctrl_DZNep_upto1hr_XZMIP_montages_nuc_MT_template.pptx"
)

# Output: explicit name (template filename contains "XZMIP" which doesn't apply here)
OUTPUT_PATH = (
    "K:/FF/PPT/PPT_autogeneration/"
    "Ctrl, DZNep, bTub, Lamin B, 20260203/"
    "Ctrl_DZNep_upto1hr_nuc_MT_montages.pptx"
)

# Primary parent directory (4min, 8min, 30min, 1hr bTub data)
PARENT_DIR = (
    "J:/FF/Nucleus_Project_up_to_1hr/"
    "020302026_jurkats_dznep_lmnB1orbTub_561LP55_488LP45_405LP40_"
)

# Secondary parent directory (15min bTub data only)
PARENT_DIR_15MIN = (
    "J:/FF/Nucleus_Project_up_to_1hr/"
    "020402026_jurkats_dznep_lmnB1orbTub_561LP55_488LP45_405LP40_"
)

# Subfolder within each condition directory containing the montage image
MONTAGE_SUBPATH = "cropped/fixed_slice_images/montages_nuc_lamin_merge"

# Glob pattern for the bTub montage (same folder/naming convention as lamin script)
IMAGE_GLOB = "fixed_slice_montage_nuc_lamin_merge_*.png"

# Only process bTub wells (skip lmnb1 directories)
MARKER_FILTER = "btub"

# Title textbox position/size (inches)
TITLE_LEFT = 0.3
TITLE_TOP = 0.1
TITLE_WIDTH = 12.7
TITLE_HEIGHT = 0.7
TITLE_FONT_SIZE = Pt(24)

# Image position (inches) — height auto-calculated to preserve aspect ratio
IMG_LEFT = 0.3
IMG_TOP = 0.9
IMG_WIDTH = 12.7

# ---------------------------------------------------------------------------


def format_condition_name(folder_name: str) -> str:
    """Convert a raw folder name to a readable slide title.

    Expected format: [replicate]_[condition]_[timepoint]_[marker]_
    Example: G1A4_h2o_4min_bTub_ → 'H2O 4 min: β-Tub'

    Falls back to the raw folder name if the pattern isn't recognised.
    """
    condition_map = {"h2o": "H2O", "dznep": "DZNep"}
    timepoint_map = {
        "4min": "4 min", "8min": "8 min", "15min": "15 min",
        "30min": "30 min", "1hr": "1 hr",
    }
    marker_map = {"lmnb1": "Lamin B1", "btub": "\u03b2-Tub"}

    parts = folder_name.strip("_").split("_")
    if len(parts) != 4:
        return folder_name

    _, condition, timepoint, marker = parts
    return (
        f"{condition_map.get(condition.lower(), condition)} "
        f"{timepoint_map.get(timepoint.lower(), timepoint)}: "
        f"{marker_map.get(marker.lower(), marker)}"
    )


def find_image(folder: Path, pattern: str) -> Path | None:
    """Return first glob match in folder, or None."""
    matches = sorted(folder.glob(pattern))
    return matches[0] if matches else None


def add_title(slide, text: str) -> None:
    """Add a centered bold title textbox to the slide."""
    txBox = slide.shapes.add_textbox(
        Inches(TITLE_LEFT), Inches(TITLE_TOP),
        Inches(TITLE_WIDTH), Inches(TITLE_HEIGHT),
    )
    tf = txBox.text_frame
    tf.text = text
    para = tf.paragraphs[0]
    para.font.size = TITLE_FONT_SIZE
    para.font.bold = True
    para.alignment = PP_ALIGN.CENTER


def main() -> None:
    # Collect bTub condition dirs from both parent directories
    condition_dirs = []
    for parent_path in (PARENT_DIR, PARENT_DIR_15MIN):
        parent = Path(parent_path)
        if not parent.exists():
            print(f"WARNING: Parent directory not found, skipping: {parent}")
            continue
        condition_dirs += [
            d for d in parent.iterdir()
            if d.is_dir() and MARKER_FILTER in d.name.lower()
        ]

    # Sort by folder name so timepoints are in order
    condition_dirs = sorted(condition_dirs, key=lambda d: d.name)

    if not condition_dirs:
        print("ERROR: No matching subdirectories found in either parent directory.")
        return

    print(f"Found {len(condition_dirs)} condition directories.")
    print(f"Opening PPT: {PPT_PATH}")
    print(f"(15min data from: {PARENT_DIR_15MIN})\n")

    prs = Presentation(PPT_PATH)
    blank_layout = prs.slide_layouts[6]  # Blank layout

    slides_added = 0
    missing_report = []

    for cond_dir in condition_dirs:
        cond_name = cond_dir.name
        montage_dir = cond_dir / MONTAGE_SUBPATH
        img = find_image(montage_dir, IMAGE_GLOB) if montage_dir.exists() else None

        img_status = img.name if img else "NOT FOUND"
        print(f"[{cond_name}]  {img_status}")

        if not montage_dir.exists():
            missing_report.append(f"{cond_name}: montage folder missing")
        elif not img:
            missing_report.append(f"{cond_name}: image not found")

        slide = prs.slides.add_slide(blank_layout)
        add_title(slide, format_condition_name(cond_name))

        if img:
            slide.shapes.add_picture(
                str(img),
                Inches(IMG_LEFT), Inches(IMG_TOP),
                width=Inches(IMG_WIDTH),
            )

        slides_added += 1

    prs.save(OUTPUT_PATH)
    print(f"\nDone. {slides_added} slides added. Saved to:\n  {OUTPUT_PATH}")

    if missing_report:
        print(f"\nMissing ({len(missing_report)}):")
        for msg in missing_report:
            print(f"  - {msg}")
    else:
        print("\nAll images found — no missing items.")


if __name__ == "__main__":
    main()

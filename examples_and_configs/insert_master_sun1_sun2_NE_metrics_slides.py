"""
insert_master_sun1_sun2_NE_metrics_slides.py

Master SUN1-vs-SUN2 deck focused on nuclear-envelope-based metrics. One slide
per metric, SUN1 (N1 vs N2) and SUN2 (siControl) side by side. Includes the
SUN × DNA (Hoechst) correlation ALONG the nuclear envelope, plus the SUN × DNA
correlation at the deepest invagination.

Both compiles are on O: (the former H: drive was remapped to O:). SUN2 is a
single condition; SUN2 has no near_cent, so that topic is omitted here.

Usage:
    conda run -n PPT_editing python examples_and_configs/insert_master_sun1_sun2_NE_metrics_slides.py
    conda run -n PPT_editing python examples_and_configs/insert_master_sun1_sun2_NE_metrics_slides.py --list
"""
...
OUTPUT_PATH = Path(
    ...
    "Master, SUN1 vs SUN2 — NE-based metrics (incl. SUN x DNA on NE).pptx")

DECK_TITLE = "SUN1 vs SUN2 — nuclear-envelope-based metrics"
DECK_SUBTITLE = (
    "Fixed Jurkats · siControl · SUN = perinuc 0.5 μm outside NE · "
    "SUN1 (N1 vs N2) & SUN2 (single condition) · one slide per metric · "
    "includes SUN × DNA correlation at the nuclear envelope")
...
# ---------------------------------------------------------------------------
SUN1_LBL = "N1 (May 18) vs N2 (May 31)"
...
def _sun1(root):
    ...
    return {
        ...
        "density": lambda g: [
            (S / "SUN1_geomdens_{}_boundary_OVERLAY_line.png".format(g),
             SUN1_LBL + " · NE (boundary)"),
            (S / "SUN1_geomdens_{}_perinuc05_OVERLAY_line.png".format(g),
             SUN1_LBL + " · perinuc 0.5 μm")],
        "enrich": lambda: [(E / "SUN1_hulldist_gt0.5um_shells.png", SUN1_LBL)],
        "enrich_min": lambda: [(E / "SUN1_mincurv_lt0_shells.png", SUN1_LBL)],
        "enrich_mean": lambda: [(E / "SUN1_meancurv_lt0_shells.png", SUN1_LBL)],
        "corr": lambda: [(E / "SUN1_corr_hulldist_shells.png", SUN1_LBL)],
        "corr_min": lambda: [(E / "SUN1_corr_mincurv_shells.png", SUN1_LBL)],
        "corr_mean": lambda: [(E / "SUN1_corr_meancurv_shells.png", SUN1_LBL)],
        "pocket": lambda: [
            (G / "SUN1_cyto_in_nuc_hull_vs_near_convex_nuc_MFI_ratio.png",
             SUN1_LBL)],
        "ne_dna_corr": lambda: [(G / "SUN1_NE_Hoechst_corr.png", SUN1_LBL)],
        "deep_dna_corr": lambda: [
            (G / "SUN1_deepest_invag_Hoechst_corr_025um.png",
             SUN1_LBL + " · 0.25 μm"),
            (G / "SUN1_deepest_invag_Hoechst_corr_05um.png",
             SUN1_LBL + " · 0.5 μm")],
        "depth": lambda: [(D / "SUN1_invag_depth_profiles.png", SUN1_LBL)],
    }
...
TOPICS = [
    ...
    ("deep_dna_corr", None, "SUN × DNA correlation in deep invaginations",
     "SUN vs DNA correlation at the deepest invagination (0.25 & 0.5 μm)"),
    ("pocket", None, "SUN in nuclear invagination pockets",
     "grooves ÷ convex nuclear surface (MFI ratio)"),
    ("depth", None, "SUN invagination depth profiles",
     "SUN signal vs depth within nuclear invaginations"),
]

CHAN_ORDER_NOTE = "SUN1 (N1 vs N2) then SUN2 (siControl)"
...
def build_plan():
    ...
    for key, geom, dtitle, dsub in TOPICS:
        ...
        if key in HUGE_TOPICS:
            ...
            plan.append(("grid", "{}  ·  SUN1 vs SUN2".format(short), combined,

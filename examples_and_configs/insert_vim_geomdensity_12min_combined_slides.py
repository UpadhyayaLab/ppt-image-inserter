"""
insert_vim_geomdensity_12min_combined_slides.py

Build the Vimentin geometry-density 12 min activation summary deck from the
20260716 combined compile.

Sections (each 1×2 BYEXP panel already overlays PLL vs aCD3 for both
...
Usage:
    conda run -n PPT_editing python examples_and_configs/insert_vim_geomdensity_12min_combined_slides.py
    conda run -n PPT_editing python examples_and_configs/insert_vim_geomdensity_12min_combined_slides.py --list
"""
...
OUTPUT_PATH = Path(
    ...
    "Effects_of_activation/"
    "Vim vs NE geometry, aCD3 vs PLL, 12 min (20260716).pptx"
)

DECK_TITLE = "Vimentin density vs nuclear-envelope geometry — 12 min activation"
DECK_SUBTITLE = (
    "Fixed Jurkats · E6-1 · aCD3 12 min (activated) vs PLL 12 min "
    "(non-activated) · vim = perinuc 0.5 μm · "
    "per experiment · compiled 2026-07-16"
)

COND_A = "aCD3_12m"
COND_B = "PLL_12m"
COND_A_DISP = "aCD3 12 min"
COND_B_DISP = "PLL 12 min"

EXP_DATES = [
    ("Dec 12 2024", "Dec_12__2024", "Dec_12,_2024", "Dec_12_2024"),
    ("Mar 18 2024", "Mar_18__2024", "Mar_18,_2024", "Mar_18_2024"),
]
...
def build_plan():
    ...
    # ===================================================================
    # Sec 1: Vim density profiles — by-experiment (1x2: Mar 18 | Dec 12)
    #        Each 1x2 panel already overlays PLL vs aCD3 for BOTH
    #        experiments, so no separate OVERLAY slides are needed.
    # ===================================================================
    plan.append(("divider",
                 "Vimentin density profiles",
                 "perinuc 0.5 μm shell · PLL vs aCD3, by experiment"))
    ...
    if kept:
        plan.append(("grid",
                     ...
                     kept,
                     "each 1×2: PLL (magenta) vs aCD3 (blue), "
                     "Mar 18 | Dec 12 · perinuc 0.5 μm",
                     3, 10, 0.28))
    ...
    plan.append(("divider",
                 ...
                 "NE + perinuc shells · per experiment · aCD3 vs PLL"))
    ...
    _add_matrix(
        ...
        "enrichment = fraction ÷ expected · aCD3 vs PLL · rows = experiment")
    ...
    _add_matrix(
        ...
        "ref line 1 · aCD3 vs PLL · rows = experiment")
    ...
    _add_matrix(
        ...
        "ref line 0 · aCD3 vs PLL · rows = experiment")
    ...
    _add_matrix(
        ...
        "deepest-invagination voxels · ref line 0 · aCD3 vs PLL · "
        "rows = experiment")
    ...
    plan.append(("divider",
                 ...
                 "centrosome-stratified · PLL vs aCD3, by experiment"))
    ...
    _add_matrix(
        ...
        "color = centrosome side · aCD3 vs PLL · rows = experiment")
    ...
    # ===================================================================
    plan.append(("divider",
                 "Vimentin scalars — aCD3 vs PLL",
                 "grid panels · aCD3 12 min vs PLL 12 min"))
    ...
    if kept:
        plan.append(("grid",
                     ...
                     kept,
                     "vim at deepest invag ÷ reference · aCD3 vs PLL · "
                     "ref line 1",
                     4, 11, 0.30))
    ...
    # ===================================================================
    # Sec 8: Histograms & QC (merged, 4 tiles)
    # ===================================================================
    plan.append(("divider",
                 "Histograms & QC",
                 "per experiment"))
    ...
    qc_tiles = []
    for disp, dbl, _comma, _sgl in EXP_DATES:
        qc_tiles.append((
            HIST / "vim_cyto_near_vs_away_deepest_invag_ratio_"
                   "histogram_{}.png".format(dbl),
            "near vs away ratio — {}".format(disp)))
    for disp, _dbl, comma, _sgl in EXP_DATES:
        qc_tiles.append((
    ...
    if kept:
        plan.append(("grid",
                     "Histograms & Simpson's paradox check",
                     kept,
                     "per experiment",
                     4, 10, 0.28))

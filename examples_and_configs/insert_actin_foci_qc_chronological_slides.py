"""
insert_actin_foci_qc_chronological_slides.py

CAT (left) vs FMC (right) side-by-side foci QC deck across every
(date, marker, timepoint) group in the compiled manifest at

    L:/FF/CAR T/actin_compiled_results/all_datasets_actin/compiled_20260623/
      dataset_manifest.csv

Each group emits TWO slides (interleaved):
    1. Foci Mask            -> <base>/<prog>/foci/foci_mask/montages/
    2. Foci Final Overlay   -> <base>/<prog>/foci/final_overlay/montages/

Rows whose neither foci dir resolves are skipped (typically TIRF
datasets and older runs that didn't get the foci pipeline). Within a
group, if only one kind resolves, that slide still renders and the
missing kind's cells show `(missing)`.

Sibling to insert_actin_synapse_mask_all_datasets_chronological_slides.py
(synapse mask) and insert_actin_xz_mip_all_datasets_chronological_slides.py
(XZ MIP). All three share the chronological-by-manifest, per-slide PPI,
CAT-left/FMC-right layout.

Usage:
    python examples_and_configs/insert_actin_foci_qc_chronological_slides.py
"""
...
OUTPUT_PATH = (
    ...
    "CART_actin_foci_QC_all_datasets_20260623.pptx"
)

# (kind label, subpath under <base>/<progress_folder>/).
# Emitted in this order, interleaved per (date, marker, tp) group.
KINDS = [
    ("Foci Mask",          "foci/foci_mask/montages"),
    ("Foci Final Overlay", "foci/final_overlay/montages"),
]
...
def resolve_kind_dir(base_dir: str, progress_folder: str, kind_subpath: str) -> Optional[Path]:
    """Resolve <base>/<prog>/<kind_subpath>/. Returns the Path if dir
    exists, else None."""
...
def main() -> None:
    ...
    # Per-row parse: for each kind, resolve montages dir + first chunk.
    # parsed[idx][kind_label] = (montages_dir or None, chunk or None)
    parsed_rows = []
    ...
    # Each group holds, for each kind, the resolved CAT and FMC dirs/chunks.
    ...
    # Filter: drop groups with NO foci data on either CAT/FMC for either kind.
    def group_has_any_foci(g) -> bool:
        for k, _ in KINDS:
            slot = g["per_kind"][k]
            if (slot["cat_chunk"] and _path_exists(slot["cat_chunk"])) or \
               (slot["fmc_chunk"] and _path_exists(slot["fmc_chunk"])):
                return True
        return False

    groups = {k: g for k, g in groups.items() if group_has_any_foci(g)}

    # Sort by (date, manifest order of first row in this group).
    sorted_groups = sorted(
        groups.items(),
        key=lambda kv: (kv[0][0], kv[1]["first_idx"]),
    )

    # Build slide specs — one slide per (group, kind), interleaved per group.
    slide_specs = []
    ...
    skipped_groups = sum(
        1 for p in parsed_rows
        if p["cell"] is not None and p["tp"] is not None
    )  # crude; we'll print better below
    print(
        f"Manifest rows: {len(rows)}\n"
        f"Groups with foci data: {n_groups}\n"

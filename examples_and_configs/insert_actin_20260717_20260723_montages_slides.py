"""
insert_actin_20260717_20260723_montages_slides.py

CAT (left) vs FMC (right) side-by-side deck for the two H:\ actin-only
CAR-T experiments (20260717 and 20260723). Grouped by kind — all 8
slides of one kind consecutively, then next kind:

    Slides  1- 8: Actin at Synapse                    (segmented 1slice mask)
    Slides  9-16: Actin: 0, 2, and 4 slices above synapse      (bottom_slices_xy raw)
    Slides 17-24: IRM Segmentation                    (coverslip-contact seg)
    Slides 25-32: Actin XZ MIP                        (raw axial)
    Slides 33-40: Actin Axial Profile            (axial total signal)
    Slides 41-48: Actin Foci Overlay                  (foci pipeline)

Within each kind block, slides are chronological by (date, tp):
20260717 3/5/7/10, then 20260723 3/5/7/10.

Both experiments live on H:\ and are NOT in the L:
`all_datasets_actin/compiled_20260623/dataset_manifest.csv`, so this
script enumerates condition folders directly instead of driving from
a CSV manifest. Structure per experiment:

    H:/<date>_.../<cond>/<cond>/converted/cropped/channels/
        prog_fixed_cells/<kind_subpath>/montage_cells_XX_YY.png

Sibling to the insert_pMLC_/insert_MT_CatB_ decks; layout/PPI/scalebar
logic copied verbatim.

Usage:
    python examples_and_configs/insert_actin_20260717_20260723_montages_slides.py
"""
...
EXPERIMENTS = [
    ("20260717", "H:/20260717_CART_3_5_7_10_actin"),
    ("20260723", "H:/20260723_fix_actin_CAR_3_5_7_10"),
]

PROGRESS_FOLDER = "prog_fixed_cells"

OUTPUT_PATH = (
    "K:/FF/PPT/PPT_autogeneration/CART/actin_only/"
    "CART_actin_montages_20260717_20260723.pptx"
)

# (kind label, subpath under <base>/<progress_folder>/).
# Grouped by kind: all 8 slides of one kind first, then next.
# All kinds use the QC-passing `montages_filtered_v2/` sibling folder
# (same `montage_cells_<first>_<last>.png` filename format as the
# regular montages/, just kept-only cells).
KINDS = [
    ("Actin at Synapse",                "actin/synapse/1slice/mask/montages_filtered_v2"),
    ("Actin: 0, 2, and 4 slices above synapse",  "actin/synapse/bottom_slices_xy/montages_filtered_v2"),
    ("IRM Segmentation",                "IRM/seg/montages_filtered_v2"),
    ("Actin XZ MIP",                    "actin/xz_mip/montages_filtered_v2"),
    ("Actin Axial Profile",        "actin/axial/total_signal/montages_filtered_v2"),
    ("Actin Foci Overlay",              "foci/final_overlay/montages_filtered_v2"),
]
...
def parse_condition(name: str) -> Tuple[Optional[str], Optional[int]]:
...
def enumerate_conditions(exp_root: str) -> List[Tuple[str, str, int]]:
    """Return list of (condition_folder_name, cell, tp) direct children of
    exp_root whose name matches (CAT|FMC)(_)?<tp>min."""
    out = []
    for name in os.listdir(exp_root):
        cell, tp = parse_condition(name)
        if cell is None or tp is None:
            continue
        full = os.path.join(exp_root, name)
        if not os.path.isdir(full):
            continue
        out.append((name, cell, tp))
    return out
...
def main() -> None:
    ...
    # Build parsed rows by walking both experiment roots directly.
    # Each "row" = one (experiment, condition-folder) = one CAT-or-FMC lane
    # for a given timepoint.
    parsed_rows = []
    for order_idx, (date_tag, exp_root) in enumerate(EXPERIMENTS):
        if not os.path.isdir(exp_root):
            print(f"WARNING: experiment root missing: {exp_root}")
            continue
        conds = enumerate_conditions(exp_root)
        # Sort within experiment for stable first_idx: CAT before FMC, then tp.
        conds.sort(key=lambda t: (t[1], t[2]))
        for local_idx, (cond_name, cell, tp) in enumerate(conds):
            base_dir = (
                f"{exp_root}/{cond_name}/{cond_name}/converted/cropped/channels"
            )
    ...
    # Group by (date, tp). CAT and FMC lanes collapse into one slide-group.
    ...
    for p in parsed_rows:
        ...
        g["first_idx"] = min(g["first_idx"], p["idx"])
        for kind_label, _ in KINDS:
    ...
    # Sort groups by (date_tag, tp) — chronological across both experiments.
    sorted_groups = sorted(
        groups.items(),
        key=lambda kv: (kv[0][0], kv[0][1]),
    )

    # Group by kind first; within kind block, iterate the sorted groups.
    slide_specs = []
    for kind_label, _ in KINDS:
        for ((date_tag, tp), g) in sorted_groups:
            slot = g["per_kind"][kind_label]
            title = f"{kind_label} — {date_tag}, {tp} min"
            log_key = f"{kind_label}/{date_tag}/{tp}min"
            slide_specs.append((
                title,

"""
insert_actin_qc_synapse_mask_xz_mip_slides.py

Actin-only QC deck: synapse mask + XZ MIP intermediate-step montages
from the CART_fixed_cell_analysis actin-only pipeline.

For each (dataset, condition, kind) combination, builds one slide
with a single full-width montage below a bold title.
...
where <base> = <dataset>/<condition>/converted/cropped/split_channels/

Within each montages/ directory we glob `montage_cells_*.png` and pick
the first sorted file (first chunk, ~25 cells).

Datasets:
    D1: 03122026_pMLC_actin_CAR_T
    D2: 20260510_pMLC_Actin561_nucleus_CAR_Tcell_
    D3: 20260414_p_PKC_theta_Phalloidin561_

Conditions: CAT_5min, CAT_10min, CAT_15min, FMC_5min, FMC_10min, FMC_15min

Result: 3 datasets x 6 conditions x 2 kinds = 36 slides.

Modeled after insert_lwi_qc_overlay_minEdgeLen_16_p01_slides.py.

Usage:
    python examples_and_configs/insert_actin_qc_synapse_mask_xz_mip_slides.py
"""
...
# (YYYYMMDD acquisition date, dataset folder name) — order = slide ordering D1 -> D2 -> D3
...
def main() -> None:
    ...
    # Loop order: kind -> dataset -> condition so all Synapse slides come
    # before all XZ MIP slides.

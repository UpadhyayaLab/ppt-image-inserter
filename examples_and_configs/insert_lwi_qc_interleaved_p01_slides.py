"""
insert_lwi_qc_interleaved_p01_slides.py

Generate a combined LWI_QC deck that interleaves part1 (anatomical
views) and part2 (wrinkle metrics) per condition. For each lmnb1
condition subdirectory, builds TWO slides back-to-back:

    Slide 2N+1 — Part 1: Overlay | Classified
                         Edges   | Raw
    Slide 2N+2 — Part 2: LWI            | SWI
                         Heatmap (LWI)  | Heatmap (SWI)

Result: 10 conditions × 2 slides = 20 slides total, in
H2O→DZNep × 4 min→1 hr order.

Same source dir, layout, and styling as
insert_lwi_qc_part1_p01_slides.py / insert_lwi_qc_part2_p01_slides.py.

Usage:
    python examples_and_configs/insert_lwi_qc_interleaved_p01_slides.py
"""
...
OUTPUT_PATH = (
    ...
    "Ctrl_DZNep_upto1hr_LWI_QC_interleaved_LaminB1_p01.pptx"
)
...
# Two kind sets in interleave order. Each entry: (label, filename),
# ordered TL → TR → BL → BR for the 2×2 grid.
IMAGE_KINDS_PART1 = [
...
IMAGE_KINDS_PART2 = [
    ("LWI",           "Montage_LWI_LaminB1_p01.png"),
    ("SWI",           "Montage_SWI_LaminB1_p01.png"),
    ("Heatmap (LWI)", "Montage_Heatmap_LaminB1_p01.png"),
    ("Heatmap (SWI)", "Montage_Heatmap_SWI_LaminB1_p01.png"),
]
PARTS = [IMAGE_KINDS_PART1, IMAGE_KINDS_PART2]

# Only process lmnb1 wells
MARKER_FILTER = "lmnb1"
...
def build_slide(prs, cond_dir: Path, kinds, title_text: str):
    ...
    for (kind, filename), (cell_left, cell_top) in zip(kinds, CELL_POSITIONS):
        add_textbox(
            slide, kind,
...
def main() -> None:
    ...
    for cond_dir in condition_dirs:
        ...
        for part_idx, kinds in enumerate(PARTS, start=1):
            _, missing = build_slide(prs, cond_dir, kinds, title_text)
            present = sum(1 for _, fn in kinds if (cond_dir / QC_SUBPATH / fn).exists())
            print(f"[{cond_dir.name}  part{part_idx}]  {present}/{len(kinds)} images present")

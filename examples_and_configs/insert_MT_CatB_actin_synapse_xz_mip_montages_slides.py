"""
insert_MT_CatB_actin_synapse_xz_mip_montages_slides.py

Multi-channel (actin + CatB + MT) montage deck across the 5 CAR-T CatB
datasets in the compiled manifest:

  Y: drive AP data (actin+CatB, no MT/nuc):
    - 20231018
    - 20240312
  J: drive (actin+MT+CatB[+future nuc]):
    - 20231127
    - 20240620
    - 20240624

Grouped by kind — all groups of one kind consecutively, chronological
within each block:

    Actin at Synapse (segmented mask)
    CatB + Actin Composite at Synapse
    MT + Actin Composite at Synapse         (missing for Y: drive datasets — no MT)
    Actin XZ MIP
    CatB XZ MIP
    MT XZ MIP                                (missing for Y: drive datasets — no MT)

Scaffold: cells render `(missing)` for conditions still being reprocessed.
Rerun this script as new results land; slide count stays constant.

Future kinds to add once reprocessing writes them (currently commented out
in KINDS):
    - <ch>/synapse/1slice/fixed_con/montages  (fixed contrast solo)
    - nuc/synapse/composite/1slice/montages
    - nuc/xz_mip/(fixed_con/)montages
    - dual MT+CatB composite (path TBD)

Filter: `marker == "CatB"` rows only. Slide count = kinds x groups.

Usage:
    python examples_and_configs/insert_MT_CatB_actin_synapse_xz_mip_montages_slides.py
"""
...
OUTPUT_PATH = (
    ...
    "CART_actin_MT_CatB_montages_20260702.pptx"
)

# Only include manifest rows whose marker matches this set.
# All 5 CatB experiments are tagged `CatB` in the manifest — MT presence
# is determined by whether the MT/ subtree resolves per condition.
MARKER_FILTER = {"CatB"}

# (kind label, subpath under <base>/<progress_folder>/).
# Grouped by kind — all groups of one kind consecutively (chronological
# within each block). Synapse views first, then XZ MIP views.
#
# Y: drive datasets (20231018, 20240312) have no MT subtree — those
# rows render `(missing)` for the MT kinds.
KINDS = [
    ("Actin at Synapse",                       "actin/synapse/1slice/mask/montages"),
    ("CatB + Actin Composite at Synapse",      "CatB/synapse/composite/1slice/montages"),
    ("MT + Actin Composite at Synapse",        "MT/synapse/composite/1slice/montages"),
    ("Actin XZ MIP",                           "actin/xz_mip/montages"),
    # Annotated XZ views — same XZ MIP but with percentile / centrosome
    # markers drawn on top.
    ("CatB XZ MIP with z25/z50/z75 lines",     "CatB/axial/binning/montages"),
    ("MT XZ MIP with centrosome marked",       "MT/centrosome/montages"),
    # Plain XZ MIPs at the end for reference.
    ("CatB XZ MIP",                            "CatB/xz_mip/montages"),
    ("MT XZ MIP",                              "MT/xz_mip/montages"),
]
...
def main() -> None:
    ...
    for kind_label, _ in KINDS:
        for ((date_tag, marker_label, tp), g) in sorted_groups:
            slot = g["per_kind"][kind_label]
            # Drop the manifest marker from the title — it's the dataset
            # label ("CatB") not the channel being shown, so leaving it in
            # was confusing on kind-based slides (e.g. "Actin XZ MIP").

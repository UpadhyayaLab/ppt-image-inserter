"""
insert_pPKC_actin_synapse_xz_mip_montages_slides.py

CAT (left) vs FMC (right) side-by-side deck for the single Kiet pPKC
confocal dataset (20260414, `20260414_p_PKC_theta_Phalloidin561_`).
Grouped by kind — all 3 slides of one kind consecutively:

    Slides  1- 3: Actin at Synapse                   (segmented)
    Slides  4- 6: pPKC at Synapse                    (fixed contrast, solo)
    Slides  7- 9: pPKC + Actin Composite at Synapse  (composite, fixed contrast)
    Slides 10-12: Actin XZ MIP
    Slides 13-15: pPKC XZ MIP                        (fixed contrast)

Within each kind block, slides are chronological by (date, tp).

Filter: `marker == "pPKC"` rows only. 1 dataset x 3 timepoints x 5 kinds
= 15 slides.

Usage:
    python examples_and_configs/insert_pPKC_actin_synapse_xz_mip_montages_slides.py
"""
...
OUTPUT_PATH = (
    "K:/FF/PPT/PPT_autogeneration/CART/pPKC/"
    "CART_pPKC_actin_synapse_xz_mip_montages_20260625.pptx"
)
...
KINDS = [
    ...
    ("pPKC at Synapse (fixed contrast)",    "pPKC/synapse/1slice/fixed_con/montages"),
    ("pPKC + Actin Composite at Synapse",   "pPKC/synapse/composite/1slice/montages"),
    ("Actin XZ MIP",                        "actin/xz_mip/montages"),
    ("pPKC XZ MIP (fixed contrast)",        "pPKC/xz_mip/fixed_con/montages"),
]

"""
insert_lwi_qc_views_p01_slides.py

Generate an anatomical-views QC deck from the LWI_QC
Montages_filter_pp_load directory under prog_lwi_swi_only.
For each lmnb1 condition subdirectory, builds one slide with a
2x2 grid of the four p01 view montages:

    Overlay    |    Classified
    Edges      |    Raw

Black background, white bold title + bold kind labels. No template
required — slides are built from scratch.

Companion to insert_lwi_qc_p01_slides.py (which shows
Overlay/Classified/Edges/Heatmap from the original `prog/` pipeline).
This script reads from the new `prog_lwi_swi_only/` pipeline and
substitutes Raw for the Heatmap quadrant.

Usage:
    python examples_and_configs/insert_lwi_qc_views_p01_slides.py
"""
...
OUTPUT_PATH = (
    ...
    "Ctrl_DZNep_upto1hr_LWI_QC_part1_LaminB1_p01.pptx"
)
...
# New minimal-pipeline output dir (vs. the original `prog/` used by the
# sibling insert_lwi_qc_p01_slides.py script).
QC_SUBPATH = "cropped/channels/prog_lwi_swi_only/LWI_QC/Montages_filter_pp_load"

# Ordered (kind label, image filename). Order = visual order in the 2x2
...
def add_image_in_cell(slide, image_path, cell_left, cell_top):
    ...
    aspect ratio. Fits to whichever cell dimension is binding."""
...
def build_condition_slide(prs, cond_dir: Path):
    """Build one QC slide for one condition. Returns (slide, missing_kinds)."""

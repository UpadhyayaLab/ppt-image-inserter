"""
insert_PKC_actin_synapse_xz_mip_montages_slides.py

CAT (left) vs FMC (right) side-by-side deck for the single Kiet PKC
confocal dataset (20260131, `01312026_fixed_CART_PKC_Phalloidin_`).
Grouped by kind — all 3 slides of one kind consecutively:

    Slides  1- 3: Actin at Synapse                   (segmented)
    Slides  4- 6: PKC at Synapse                     (fixed contrast, solo)
    Slides  7- 9: PKC + Actin Composite at Synapse   (composite, fixed contrast)
    Slides 10-12: Actin XZ MIP
    Slides 13-15: PKC XZ MIP                         (fixed contrast)

Within each kind block, slides are chronological by (date, tp).

Filter: `marker == "PKC"` rows only. 1 dataset x 3 timepoints x 5 kinds
= 15 slides.

Note: the PKC dataset base_dirs have an extra nested `<cond>_/` folder
(e.g. `.../CAT_5min/CAT_5min_/converted/...`). The manifest already
encodes the full base_dir including that segment, so no code path change
is needed here vs the pMLC script — same resolve_kind_dir logic works.

Usage:
    python examples_and_configs/insert_PKC_actin_synapse_xz_mip_montages_slides.py
"""
...
OUTPUT_PATH = (
    "K:/FF/PPT/PPT_autogeneration/CART/PKC/"
    "CART_PKC_actin_synapse_xz_mip_montages_20260625.pptx"
)
...
KINDS = [
    ...
    ("PKC at Synapse (fixed contrast)",    "PKC/synapse/1slice/fixed_con/montages"),
    ("PKC + Actin Composite at Synapse",   "PKC/synapse/composite/1slice/montages"),
    ("Actin XZ MIP",                        "actin/xz_mip/montages"),
    ("PKC XZ MIP (fixed contrast)",        "PKC/xz_mip/fixed_con/montages"),
]

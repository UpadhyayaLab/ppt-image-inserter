"""
insert_lwi_qc_p01_slides.py

Generate a QC deck from the LWI_QC Montages_filter_pp_load directory.
For each lmnb1 condition subdirectory of the experiment, builds one
slide showing the four p01 QC montages in a 2x2 grid:

    Boundary    |    Classified
    Edges       |    Heatmap

Black background, white title + kind labels. No template required —
slides are built from scratch.

Usage:
    python examples_and_configs/insert_lwi_qc_p01_slides.py
"""
...
# Add repo root to path (parallel to the other insert_*_slides.py scripts).
...
OUTPUT_PATH = (
    ...
    "Ctrl_DZNep_upto1hr_LWI_QC_LaminB1_p01.pptx"
)
...
# Within each condition directory
QC_SUBPATH = "cropped/channels/prog/LWI_QC/Montages_filter_pp_load"

# Ordered list of (kind label, image filename). Order = visual order in
# the 2x2 grid (top-left, top-right, bottom-left, bottom-right).
IMAGE_KINDS = [
    ("Overlay",    "Montage_Overlay_LaminB1_p01.png"),
    ("Classified", "Montage_Classified_LaminB1_p01.png"),
    ("Edges",      "Montage_Edges_LaminB1_p01.png"),
    ("Heatmap",    "Montage_Heatmap_LaminB1_p01.png"),
]
...
# Slide layout (inches). Defaults assume 13.333 x 7.5 widescreen.
SLIDE_W = 13.333
SLIDE_H = 7.5
...
# 2x2 grid cells (each cell holds a label + image)
CELL_W = 6.55
CELL_H = 3.40
LABEL_H = 0.25
IMG_H = CELL_H - LABEL_H   # 3.15
LABEL_FONT_PT = 14

# Cell origins (top-left of each cell)
GRID_LEFT = 0.10
GRID_TOP = 0.55
COL_GAP = SLIDE_W - 2 * GRID_LEFT - 2 * CELL_W   # gap between TL/TR
ROW_GAP = SLIDE_H - GRID_TOP - 2 * CELL_H - 0.1  # small bottom margin

CELL_POSITIONS = [
    (GRID_LEFT,                       GRID_TOP),                       # TL: Boundary
    (GRID_LEFT + CELL_W + COL_GAP,    GRID_TOP),                       # TR: Classified
    (GRID_LEFT,                       GRID_TOP + CELL_H + ROW_GAP),    # BL: Edges
    (GRID_LEFT + CELL_W + COL_GAP,    GRID_TOP + CELL_H + ROW_GAP),    # BR: Heatmap
]
...
def add_image_in_cell(slide, image_path, cell_left, cell_top):
    """Place an image inside a cell (below the label area), preserving
    aspect ratio by fixing height (so the image always fits vertically;
    width may be < CELL_W). Centered horizontally inside the cell."""
    # Use both width and height bound to fit cleanly inside the cell.
    # python-pptx fits to whichever dimension is given; we want it to
    # fit *inside* CELL_W x IMG_H, so pick the smaller scale.
    # Easiest: add with width=CELL_W, then if the resulting height
    # overflows IMG_H, re-fit by height.
    ...
    if actual_h_in > IMG_H:
        # Remove and re-add fit by height instead.
        sp = pic._element
        ...
        # Center horizontally inside cell
...
def build_condition_slide(prs, cond_dir: Path):
    """Build a single QC slide for one condition. Returns (slide, missing_kinds)."""
    ...
    for (kind, filename), (cell_left, cell_top) in zip(IMAGE_KINDS, CELL_POSITIONS):
        ...
        if img_path.exists():
            ...
            # Drop a "MISSING" label where the image would have been
            add_textbox(
                slide, "(missing)",
...
def main() -> None:
    ...
    # Build a new presentation from scratch (no template).
    prs = Presentation()

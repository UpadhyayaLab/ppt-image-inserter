"""
insert_pLAT_solo_and_composite_synapse_montages_slides.py

CAT (left) vs FMC (right) side-by-side deck interleaving two kinds per
(date, tp) group:

    Slide 2N+1: pLAT alone at synapse (fixed contrast)
    Slide 2N+2: pLAT + Actin composite at synapse (fixed contrast)

Filter: `marker == "pLAT"` rows only (3 datasets: 20250418, 20250509,
20250611). 6 groups × 2 kinds = 12 slides (one CAT slot on 20250509 5min
is the known missing condition and renders as `(missing)`).

Combines insert_pLAT_solo_synapse_montages_slides.py and
insert_pLAT_composite_synapse_montages_slides.py into one interleaved
deck.

Usage:
    python examples_and_configs/insert_pLAT_solo_and_composite_synapse_montages_slides.py
"""
...
OUTPUT_PATH = (
    "K:/FF/PPT/PPT_autogeneration/CART/pLAT/"
    "CART_pLAT_solo_and_composite_montages_20260625.pptx"
)
...
# Interleaved per group: solo first, composite second.
KINDS = [
    ("pLAT at Synapse (fixed contrast)",  "pLAT/synapse/1slice/fixed_con/montages"),
    ("pLAT + Actin Composite at Synapse", "pLAT/synapse/composite/1slice/montages"),
]

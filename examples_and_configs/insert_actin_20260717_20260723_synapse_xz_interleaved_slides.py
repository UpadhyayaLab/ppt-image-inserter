"""
insert_actin_20260717_20260723_synapse_xz_interleaved_slides.py

Interleaved variant of insert_actin_20260717_20260723_montages_slides.py
— same two H:\ CAR-T actin experiments, same CAT-vs-FMC per-slide
layout, but INTERLEAVED per (date, tp) group instead of grouped by
kind:

    Slide  1: Actin at Synapse                — 20260717, 3 min
    Slide  2: Actin: 0, 2, and 4 slices above synapse  — 20260717, 3 min
    Slide  3: IRM Segmentation                — 20260717, 3 min
    Slide  4: Actin XZ MIP                    — 20260717, 3 min
    Slide  5: Actin Axial Profile        — 20260717, 3 min
    Slide  6: Actin at Synapse                — 20260717, 5 min
    ...

5 kinds × 8 (date, tp) groups = 40 slides.

Sibling to the grouped-by-kind variant. All helper code and layout
constants are copied verbatim — only KINDS and the slide-emit loop
differ.

Usage:
    python examples_and_configs/insert_actin_20260717_20260723_synapse_xz_interleaved_slides.py
"""
...
OUTPUT_PATH = (
    ...
    "CART_actin_synapse_xz_interleaved_20260717_20260723.pptx"
)

# (kind label, subpath under <base>/<progress_folder>/).
# Interleaved per (date, tp): synapse first, XZ MIP second, per group.
...
def main() -> None:
    ...
    # Sort groups by (date_tag, tp).
    sorted_groups = sorted(
        groups.items(),
        key=lambda kv: (kv[0][0], kv[0][1]),
    )

    # INTERLEAVED: for each group, emit all KINDS back-to-back, in KINDS order.
    slide_specs = []
    for ((date_tag, tp), g) in sorted_groups:
        for kind_label, _ in KINDS:
            slot = g["per_kind"][kind_label]
            title = f"{kind_label} — {date_tag}, {tp} min"
            log_key = f"{date_tag}/{tp}min/{kind_label}"
            slide_specs.append((
                title,
    ...
    print(
        ...
        f"Slides emitted: {len(slide_specs)} ({n_groups} groups x {len(KINDS)} kinds, interleaved)\n"

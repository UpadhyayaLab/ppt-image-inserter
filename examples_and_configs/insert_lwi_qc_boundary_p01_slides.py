"""
insert_lwi_qc_boundary_p01_slides.py

Generate a Boundary-only QC deck from the LWI_QC
Montages_filter_pp_load directory. For each lmnb1 condition
subdirectory, builds one slide with a single large Boundary
montage (p01) below a bold title.

Companion to insert_lwi_qc_p01_slides.py (which shows all four
QC kinds in a 2x2 grid). This script gives Boundary the full
slide so it's considerably larger.

Usage:
    python examples_and_configs/insert_lwi_qc_boundary_p01_slides.py
"""
...
OUTPUT_PATH = (
    ...
    "Ctrl_DZNep_upto1hr_LWI_QC_Boundary_LaminB1_p01.pptx"
)
...
IMAGE_FILENAME = "Montage_Boundary_LaminB1_p01.png"
...
def add_image_in_box(slide, image_path, box_left, box_top, box_w, box_h):
    ...
    if actual_h_in > box_h:
        # Doesn't fit by width — refit by height instead.
        sp = pic._element
...
def build_condition_slide(prs, cond_dir: Path):
    """Build a single Boundary slide for one condition.

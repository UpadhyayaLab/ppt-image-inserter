...
ROOT = Path(
    ...
    "results_compilation_Vim_MG_geomdensity_20260713"
)
...
OUTPUT_PATH = Path(
    ...
    "nuc_mesh_struct_outside_nuc/Vimentin invagination-centrosome scalars, PLL vs aCD3 (20240318).pptx"
)

DECK_TITLE = "Vimentin invagination & centrosome scalars — PLL vs aCD3"
DECK_SUBTITLE = (
    "Fixed Jurkats · 03/18/2024 · PLL 12m vs aCD3 12m · Vimentin = cytoplasmic (struct_out_nuc) · "
    "per-cell scalar comparison violins (mesh-face geom-density not in this compile) · compiled 2026-07-13"
)
...
def build_plan():
    ...
    # --- Vimentin invagination & centrosome scalars, PLL-vs-aCD3 (12 min)
    # COMPARISON violins from grid_panels/. This compile has only the per-cell
    # scalars (via grid_panels); the mesh-face geom-density curated set
    # (enrichment/correlation/density profiles/deepcorr/near_cent_level) was NOT
    # generated, so those sections above resolve empty and drop out. ---
    ...
    plan.append(("divider", "Vimentin invagination & centrosome scalars",
                 "PLL 12m vs aCD3 12m comparison violins · one dot per cell · ref line 1"))
    ...
    missing += miss
    if kept:
        plan.append(("grid", "Vimentin in the nuclear invagination pockets — PLL vs aCD3", kept,
    ...
    missing += miss
    if kept:
        plan.append(("grid", "Vimentin in the nuclear convex hull — supporting metrics — PLL vs aCD3", kept,
                     "PLL 12m vs aCD3 12m · ref line 1", 3, 11, 0.30))
    ...
    missing += miss
    if kept:
        plan.append(("grid", "Vimentin at the deepest invagination — PLL vs aCD3", kept,
    ...
    missing += miss
    if kept:
        plan.append(("grid", "Vimentin near the centrosome (NE-facing) — PLL vs aCD3", kept,
    ...
    missing += miss
    if kept:
        plan.append(("grid", "Vimentin clustered near the centrosome (cytoplasmic pool) — PLL vs aCD3", kept,
                     "3D ball around the centrosome, not NE-restricted", 2, 12, 0.30))

    # --- Centrosome ↔ nucleus axis metrics (centrosome_axis_between_metrics/),
    # PLL-vs-aCD3 comparison violins. Field names shown per panel (captions empty
    # -> the stem is the label). Chunked 6-up. ---
    CAX = ROOT / "centrosome_axis_between_metrics"

    def add_chunked(title, stems, footer, per=6):
        present, miss = _tiles([(CAX / (s + "_grid.png"), "") for s in stems])
        missing.extend(miss)
        parts = [present[i:i + per] for i in range(0, len(present), per)]
        for j, part in enumerate(parts):
            suff = "" if len(parts) == 1 else "  ({}/{})".format(j + 1, len(parts))
            plan.append(("grid", title + suff, part, footer, min(3, len(part)), 10, 0.28))

    plan.append(("divider", "Vimentin centrosome ↔ nucleus axis metrics",
                 "PLL 12m vs aCD3 12m comparison violins · one dot per cell"))
    add_chunked("Centrosome ↔ nucleus positioning", [
        "cent_dist_to_nuc_periphery", "cent_to_nuc_surf_dist",
        "vim_cent_nuc_cof_polarity_norm", "vim_cent_nuc_cof_polarity_um",
        "vim_cent_nuc_facing_index_1um", "vim_cent_nuc_facing_index_2um",
        "vim_cent_nuc_frac_sphere_in_nucleus_1um", "vim_cent_nuc_frac_sphere_in_nucleus_2um",
        "vim_cent_nuc_interposition_frac_1um", "vim_cent_nuc_interposition_frac_2um",
        "vim_cent_nuc_n_valid_distal_1um", "vim_cent_nuc_n_valid_distal_2um",
        "vim_cent_nuc_n_valid_facing_1um", "vim_cent_nuc_n_valid_facing_2um",
    ], "centrosome_axis_between_metrics/ · PLL 12m vs aCD3 12m")
    add_chunked("Vimentin clump vs centrosome / nucleus", [
        "cent_dist_to_vim_clump_1um", "cent_dist_to_vim_clump_2um",
        "cent_vim_clump_nuc_edge_dist_ratio_1um", "cent_vim_clump_nuc_edge_dist_ratio_2um",
        "vim_clump_around_cent_dist_to_nuc_periphery_1um", "vim_clump_around_cent_dist_to_nuc_periphery_2um",
        "vim_clump_around_cent_is_biggest_1um", "vim_clump_around_cent_is_biggest_2um",
        "vim_clump_around_cent_is_biggest_loose_1um", "vim_clump_around_cent_is_biggest_loose_2um",
        "vim_clump_closer_to_nuc_1um", "vim_clump_closer_to_nuc_2um",
    ], "centrosome_axis_between_metrics/ · PLL 12m vs aCD3 12m")

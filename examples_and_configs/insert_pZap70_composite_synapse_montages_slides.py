"""
insert_pZap70_composite_synapse_montages_slides.py

CAT (left) vs FMC (right) side-by-side deck of the
pZap70/synapse/composite/1slice/ panels (actin green + pZap70 magenta,
fixed contrast) across every pZap70 (date, tp) group in the compiled
manifest.

Filter: `marker == "pZap70"` rows only (3 datasets: 20230921, 20231003,
20250217 D3+D5). 6 groups -> 6 slides (single kind, no interleaving).

Sibling to insert_actin_CatB_synapse_qc_chronological_slides.py — same
per-slide PPI, CAT-left/FMC-right layout, chronological-by-manifest
ordering.

Usage:
    python examples_and_configs/insert_pZap70_composite_synapse_montages_slides.py
"""
...
OUTPUT_PATH = (
    ...
    "CART_pZap70_montages_20260625.pptx"
)
...
# Single kind: pZap70 + actin composite at synapse (fixed contrast).
KINDS = [

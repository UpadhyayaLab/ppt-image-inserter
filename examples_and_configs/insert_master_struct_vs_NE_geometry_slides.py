"""
insert_master_struct_vs_NE_geometry_slides.py

Master cross-protein deck: vimentin, microtubules (MT), SUN1, SUN2 vs
nuclear-envelope geometry. Interleaved topic by topic — for each topic there
is a divider, then one slide per channel (Vimentin, MT, SUN1, SUN2), each
showing that channel's own native comparison for the topic.

Comparison axes differ by channel and are shown as-is (nothing pooled):
  Vimentin : DMSO vs Noco, 3 experiments (Apr 29 2022, Jan 23, Feb 27 2024)
  MT       : DMSO vs Noco, 1 experiment (Jan 23 2024)
  SUN1     : N1 vs N2 (siControl)
  SUN2     : siControl (single condition)

Sources live on four drives (J:, M:, O:). SUN1/SUN2 are on O: (the former
H: drive was remapped to O:).

Usage:
    conda run -n PPT_editing python examples_and_configs/insert_master_struct_vs_NE_geometry_slides.py
    conda run -n PPT_editing python examples_and_configs/insert_master_struct_vs_NE_geometry_slides.py --list
"""
...
OUTPUT_PATH = Path(
    ...
    "Master, structural proteins vs NE geometry (vim, MT, SUN1, SUN2).pptx")

DECK_TITLE = "Structural proteins vs nuclear-envelope geometry — master"
DECK_SUBTITLE = (
    "Fixed Jurkats · vimentin, microtubules, SUN1, SUN2 · perinuc 0.5 μm · "
    "interleaved by topic (one slide per channel) · each channel's own "
    "comparison, nothing pooled")

VIM_DATES = [("Apr 29 2022", "Apr_29,_2022"),
...
def _vim(root):
    ...
    def per_exp(fmt):
        return [(E / fmt.format(d), disp) for disp, d in VIM_DATES]

    return {
        "name": "Vimentin",
        "sub": "DMSO vs Noco · 3 experiments",
        "density": lambda g: [
            (S / "vim_geomdens_{}_perinuc05_BYEXP_line.png".format(g),
             "Apr 29 2022 · Jan 23 2024 · Feb 27 2024")],
        # Vim has 3 experiments — show them per experiment (nothing pooled).
        "enrich": lambda: [
            (E / "vim_hulldist_gt0.5um_shells_{}.png".format(d), disp)
            for disp, d in VIM_DATES],
        "corr": lambda: [
        ...
        "nearcent": lambda: [
            ...
            for disp, d in VIM_DATES],
        "depth": lambda: [
            (D / "vim_invag_depth_0_5um_BYEXP_line.png",
             "Apr 29 2022 · Jan 23 2024 · Feb 27 2024")],
    }
...
def _mt(root):
    ...
    return {
        "name": "Microtubules",
        "sub": "DMSO vs Noco · Jan 23 2024",
        "density": lambda g: [
            (S / "MT_geomdens_{}_perinuc05_OVERLAY_line_Jan_23__2024.png".format(g),
             "Jan 23 2024")],
        "enrich": lambda: [
            (E / "MT_hulldist_gt0.5um_shells_Jan_23,_2024.png", "Jan 23 2024")],
        "corr": lambda: [
            (E / "MT_corr_hulldist_shells_Jan_23,_2024.png", "Jan 23 2024")],
        "pocket": lambda: [
            (G / "MT_cyto_in_nuc_hull_vs_near_convex_nuc_MFI_ratio_grid.png",
             "Jan 23 2024")],
        "nearcent": lambda: [
            (NC / "MT_near_cent_level_Jan_23,_2024.png", "Jan 23 2024")],
        "depth": lambda: [
            (D / "MT_invag_depth_profiles_Jan_23__2024.png", "Jan 23 2024")],
    }
...
def _sun1(root):
    ...
    E = root / "geom_density" / "enrichment"
    NC = root / "geom_density" / "near_cent"
    G = root / "grid_panels" / "N1_vs_N2"
    D = root / "invag_depth_profiles"
    return {
        "name": "SUN1",
        "sub": "N1 vs N2 · siControl",
        "density": lambda g: [
            (S / "SUN1_geomdens_{}_perinuc05_OVERLAY_line.png".format(g),
             "N1 (May 18 2022) vs N2 (May 31 2022)")],
        "enrich": lambda: [(E / "SUN1_hulldist_gt0.5um_shells.png",
                            "N1 (May 18) vs N2 (May 31)")],
        "corr": lambda: [(E / "SUN1_corr_hulldist_shells.png",
                          "N1 (May 18) vs N2 (May 31)")],
        "pocket": lambda: [
            (G / "SUN1_cyto_in_nuc_hull_vs_near_convex_nuc_MFI_ratio.png",
             "N1 (May 18) vs N2 (May 31)")],
        "nearcent": lambda: [(NC / "SUN1_near_cent_level.png",
                              "N1 (May 18) vs N2 (May 31)")],
        "depth": lambda: [(D / "SUN1_invag_depth_profiles.png",
                           "N1 (May 18) vs N2 (May 31)")],
    }
...
def _sun2(root):
    ...
    return {
        ...
        "density": lambda g: [
            (S / "SUN2_geomdens_{}_perinuc05_siControl_all_line.png".format(g),
             "Jul 04 2022 · siControl")],
        "enrich": lambda: [(E / "SUN2_hulldist_gt0.5um_shells.png",
                            "Jul 04 2022 · siControl")],
        "corr": lambda: [(E / "SUN2_corr_hulldist_shells.png",
                          "Jul 04 2022 · siControl")],
        "pocket": lambda: [
            (V / "SUN2_cyto_in_nuc_hull_vs_near_convex_nuc_MFI_ratio.png",
             "Jul 04 2022 · siControl")],
        "nearcent": lambda: [],   # no near_cent for SUN2
        "depth": lambda: [(D / "SUN2_invag_depth_profiles.png",
                           "Jul 04 2022 · siControl")],
    }


CHANNELS = [_vim(VIM_ROOT), _mt(MT_ROOT), _sun1(SUN1_ROOT), _sun2(SUN2_ROOT)]
...
CHAN_ORDER_NOTE = "Vimentin → MT → SUN1 → SUN2"

# Topics whose figures are dense multi-panel "monsters" — keep one channel per
# slide (full width) instead of condensing all channels onto a single slide.
HUGE_TOPICS = {"depth"}
...
def build_plan():
    ...
    for key, geom, dtitle, dsub in TOPICS:
        ...
        if key in HUGE_TOPICS:
            ...
            plan.append(("grid", "{}  ·  all channels".format(short), combined,

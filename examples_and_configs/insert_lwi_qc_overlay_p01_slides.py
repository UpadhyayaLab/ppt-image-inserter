"""
insert_lwi_qc_overlay_p01_slides.py

Generate an Overlay-only QC deck from the LWI_QC
Montages_filter_pp_load directory. For each lmnb1 condition
subdirectory, builds one slide with a single large Overlay
montage (p01) below a bold title.

Companion to insert_lwi_qc_boundary_p01_slides.py — only the
source image filename and output path differ.

Usage:
    python examples_and_configs/insert_lwi_qc_overlay_p01_slides.py
"""
...
OUTPUT_PATH = (
    ...
    "Ctrl_DZNep_upto1hr_LWI_QC_Overlay_LaminB1_p01.pptx"
)

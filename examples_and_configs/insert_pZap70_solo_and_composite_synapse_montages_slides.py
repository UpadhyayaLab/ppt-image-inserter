"""
insert_pZap70_solo_and_composite_synapse_montages_slides.py

CAT (left) vs FMC (right) side-by-side deck interleaving two kinds per
(date, tp) group:

    Slide 2N+1: pZap70 alone at synapse (fixed contrast)
    Slide 2N+2: pZap70 + Actin composite at synapse (fixed contrast)

Filter: `marker == "pZap70"` rows only (3 datasets: 20230921, 20231003,
20250217 D3+D5). 6 groups × 2 kinds = 12 slides.

Combines insert_pZap70_solo_synapse_montages_slides.py and
insert_pZap70_composite_synapse_montages_slides.py into one interleaved
deck.

Usage:
    python examples_and_configs/insert_pZap70_solo_and_composite_synapse_montages_slides.py
"""
...
OUTPUT_PATH = (
    "K:/FF/PPT/PPT_autogeneration/CART/pZap70/"
    "CART_pZap70_solo_and_composite_montages_20260625.pptx"
)

# Only include manifest rows whose marker matches this set.
MARKER_FILTER = {"pZap70"}

# (kind label, subpath under <base>/<progress_folder>/).
# Interleaved per group: solo first, composite second.
KINDS = [
    ("pZap70 at Synapse (fixed contrast)",  "pZap70/synapse/1slice/fixed_con/montages"),
    ("pZap70 + Actin Composite at Synapse", "pZap70/synapse/composite/1slice/montages"),
]

"""
insert_vim_restyled_PLL_vs_aCD3_12min_slides.py

Vimentin geom-density deck from the RESTYLED figures — PLL 12 min (magenta) vs
aCD3 12 min (blue), sig stars. 2 experiments (Mar 18 2024, Dec 12 2024) shown
SEPARATELY / across-experiment — NO pooled figures. Same layout engine and
per-experiment date-box style as the vim Ctrl-vs-Noco restyled deck, plus the
near-centrosome enrichment + fraction key readouts for this activation dataset.

Source: results_compilation_Vim_MG_geomdensity_combined_20260716/restyled

Usage:
    conda run -n PPT_editing python examples_and_configs/insert_vim_restyled_PLL_vs_aCD3_12min_slides.py
    conda run -n PPT_editing python examples_and_configs/insert_vim_restyled_PLL_vs_aCD3_12min_slides.py --list
"""
...
RESTYLED = Path(
    ...
    "results_compilation_Vim_MG_geomdensity_combined_20260716/restyled")
...
INDIV = RESTYLED / "individual_plots"

# per-experiment tokens (comma form used by enrichment / near_cent / individual)
EXP_DATES = [("Mar 18 2024", "Mar_18,_2024"),
             ("Dec 12 2024", "Dec_12,_2024")]

OUTPUT_PATH = Path(
    "K:/FF/PPT/PPT_autogeneration/Fixed Jurkats, Miscellaneous/"
    "nuc_mesh_struct_outside_nuc/"
    "Vimentin geom-density vs NE geometry, PLL vs aCD3 12 min "
    "(restyled, 20260728).pptx")

DECK_TITLE = "Vimentin vs nuclear-envelope geometry — PLL vs aCD3 12 min (restyled)"
DECK_SUBTITLE = (
    "Fixed Jurkats · PLL 12 min (magenta) vs aCD3 12 min (blue) · 2 experiments "
    "(Mar 18 2024, Dec 12 2024) shown SEPARATELY — no pooling · "
    "restyled figures · 2026-07-28")

FOOTER_CMP = "PLL 12 min (magenta) vs aCD3 12 min (blue)"
...
def build_plan():
    ...
    # NOTHING POOLED — every slide shows the 2 experiments (BYEXP tiled figure
    # or the 2 per-experiment figures side by side). per-experiment tiles get a
    # date box (header) above the plot: (path, caption=None, header=date).
    BYEXP_DIR = PROF / "singles"
    EXP_LABELS = [disp for disp, _ in EXP_DATES]  # boxes over the sub-panels
    PANELS = "panels: " + " | ".join(EXP_LABELS)
    ...
    # --- Sec 2: per-experiment enrichment (2 side by side) ---
    plan.append(("divider",
    ...
    add_grid("Vimentin centrosomal enrichment (within 0.5 μm nuc & 2 μm cent) — "
             ...
             "mean vim over the whole 0.5 μm shell · ref line 1 · " + PANELS
             + " · " + FOOTER_CMP)

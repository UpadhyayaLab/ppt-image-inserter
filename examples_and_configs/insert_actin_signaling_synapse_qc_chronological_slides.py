"""
insert_actin_signaling_synapse_qc_chronological_slides.py

CAT (left) vs FMC (right) side-by-side actin-synapse-mask + signaling-channel-at-synapse
deck across every pZap70 / pLAT / pMLC group in the compiled manifest.

Filter: `marker in {pZap70, pLAT, pMLC}` only. Per group emits TWO slides:

    Slide 2N+1: Actin at Synapse (CAT | FMC)
    Slide 2N+2: <marker> at Synapse (CAT | FMC)

The second kind's path is dynamic per row: `<marker>/synapse/1slice/raw/montages`.

Sibling to insert_actin_CatB_synapse_qc_chronological_slides.py — same
chronological-by-manifest ordering, per-slide PPI, 1x2 CAT/FMC layout.

Usage:
    python examples_and_configs/insert_actin_signaling_synapse_qc_chronological_slides.py
"""
...
OUTPUT_PATH = (
    "K:/FF/PPT/PPT_autogeneration/CART/actin_signaling_synapse/"
    "CART_actin_signaling_synapse_QC_all_datasets_20260623.pptx"
)

# Only include manifest rows whose marker matches this set.
MARKER_FILTER = {"pZap70", "pLAT", "pMLC"}

# Static first kind: actin mask. Second kind is derived per-row from the
# `marker` column: `<marker>/synapse/1slice/raw/montages`.
ACTIN_KIND_LABEL   = "Actin at Synapse"
ACTIN_KIND_SUBPATH = "actin/synapse/1slice/mask/montages"
SIGNAL_KIND_LABEL_FMT   = "{marker} at Synapse"
SIGNAL_KIND_SUBPATH_FMT = "{marker}/synapse/1slice/raw/montages"
...
def main() -> None:
    ...
    for idx, row in enumerate(rows):
        ...
        # Two kinds: actin (static path) and signaling (dynamic per marker).
        signal_kind_label   = SIGNAL_KIND_LABEL_FMT.format(marker=marker)
        signal_kind_subpath = SIGNAL_KIND_SUBPATH_FMT.format(marker=marker)
        kind_pairs = [
            (ACTIN_KIND_LABEL,  ACTIN_KIND_SUBPATH),
            (signal_kind_label, signal_kind_subpath),
        ]

        per_kind: Dict[str, Tuple[Optional[Path], Optional[Path]]] = {}
        for k_label, k_subpath in kind_pairs:
            mdir = resolve_kind_dir(base_dir, progress_folder, k_subpath)
            chunk = find_first_chunk(mdir)
            per_kind[k_label] = (mdir, chunk)
        ...
        parsed_rows.append({
            ...
            "tp": tp,
            "per_kind": per_kind,
            "kind_pairs": kind_pairs,  # ordered list of (label, subpath)
        })

    # Group by (date, marker_label, tp). Two slides per group (interleaved).
    ...
    for p in parsed_rows:
        ...
        g = groups.setdefault(key, {
            "kind_labels": [lbl for lbl, _ in p["kind_pairs"]],
            "per_kind": {
                lbl: {
                    "cat_chunk": None, "fmc_chunk": None,
                    "cat_dir": None, "fmc_dir": None,
                }
                for lbl, _ in p["kind_pairs"]
            },
            "first_idx": p["idx"],
            "modality": p["modality"],
        })
        for kind_label, _ in p["kind_pairs"]:
            mdir, chunk = p["per_kind"][kind_label]
            slot = g["per_kind"].setdefault(kind_label, {
                "cat_chunk": None, "fmc_chunk": None,
                "cat_dir": None, "fmc_dir": None,
            })
            if p["cell"] == "CAT":
    ...
    for ((date_tag, marker_label, tp), g) in sorted_groups:
        for kind_label in g["kind_labels"]:
    ...
    print(
        ...
        f"Groups: {n_groups}\n"
        f"Slides emitted: {len(slide_specs)} (2 kinds x {n_groups} groups)\n"

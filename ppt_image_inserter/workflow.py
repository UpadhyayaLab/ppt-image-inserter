...
from .slide_utils import duplicate_slide, remove_pictures_from_slide
...
GENERATED_LABEL_SHAPE_NAME = "ppt_image_inserter_generated_label"
GENERATED_LABEL_DESCRIPTION = "Generated by ppt_image_inserter; safe to remove"
...
def _get_cnvpr(shape):
    """Return the shape's non-visual property element, if available."""
    try:
...
def _mark_as_generated_label(shape) -> None:
    """Mark a text shape as a generated label so later runs can remove only it."""
    try:
        shape.name = GENERATED_LABEL_SHAPE_NAME
    except Exception:
        pass

    c_nv_pr = _get_cnvpr(shape)
    if c_nv_pr is not None:
        c_nv_pr.set("descr", GENERATED_LABEL_DESCRIPTION)
        c_nv_pr.set("title", GENERATED_LABEL_SHAPE_NAME)


def _is_generated_label(shape) -> bool:
    """Return True when a shape is one of this package's removable labels."""
    if not getattr(shape, "has_text_frame", False):
        return False

    try:
        if shape.name == GENERATED_LABEL_SHAPE_NAME:
            return True
    except Exception:
        pass
    ...
    return (
        c_nv_pr.get("descr") == GENERATED_LABEL_DESCRIPTION
        or c_nv_pr.get("title") == GENERATED_LABEL_SHAPE_NAME
    )


def _remove_generated_labels(slide: Slide) -> int:
    """Remove only labels previously generated by this package."""
    labels_removed = 0

    for shape in list(slide.shapes):
        if not _is_generated_label(shape):
            continue
...
def _add_text_label(
    ...
    try:
        ...
        _mark_as_generated_label(textbox)
...
def copy_slide_replace_images(ppt_path, source_slide_index, new_image_paths, positions=None,
    ...
    3. Removes any generated metadata label from the duplicated slide
    4. Inserts multiple new images at specified positions
    5. Optionally stores metadata in each picture's alt text
    6. Optionally adds visible text labels (disabled by default for multi-image slides)
    ...
    # Remove only labels generated by this package; preserve template text.
    num_text_removed = _remove_generated_labels(new_slide)
...
def replace_image_on_existing_slide(ppt_path: str, slide_index: int, new_image_path: str,
    ...
    3. Removes any generated text labels (to avoid duplicates)
    ...
    # Remove only generated labels to avoid duplicates; preserve manual text.
    print(f"Removing generated text labels...")
    labels_removed = _remove_generated_labels(slide)
    print(f"Removed {labels_removed} generated label(s)")
...
def add_label_to_existing_slide(ppt_path: str, slide_index: int, base_dir: str = None) -> bool:
    ...
    slide_w = prs.slide_width.inches
    slide_h = prs.slide_height.inches
    _remove_generated_labels(slide)
